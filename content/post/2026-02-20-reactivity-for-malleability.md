...
This post explains why reactivity is a key requirement for openly authored, malleable substrates, and motivate [`fluid.cell`](/docs/fluid-signals), 
...
expanding their domain. I explain the core reactive competencies of _glitch freedom_ and _early cutoff_, and explain
...
expanded capabilites are vital to support successful substrates. The post finishes with a live demo of the reactive
library.
...
and maintained, the substrate also needs to reflect the different needs of different audiences, sometimes simultaneously.
...
the development of the [`fluid.cell`](/docs/fluid-signals) which is motivated on this page, a reactive library designed
to support the needs of a malleable substrate, which preserves the good properties of commodity reactive systems whilst
...
calls for many of the properties of the reactive system which has now been built. Documentation for the [`fluid.cell`](/docs/fluid-signals)
system which I motivate here is available on [this site](/docs/fluid-signals), and a live demonstration of the system can
be seen at the [bottom of the page](#live-demo).
...
However, apart from the disclosure process, it's highly desirable that reactivity is available throughout the lifecycle
of a substrate. 
...
the system should adapt gracefully and immediately to whatever gestures the author has made to modify it. This contributes
...
since they also inherently suffer from glitches as I'll describe later, they lie outside our interest. 
...
it provides these guarantees compared to commodity systems, to meet the needs of substrates. Some of this journey is described in 
my work in progress posting [understanding reactivity](/wip/2025-12-15-understanding-reactivity).
...
a core of 3 nodes, with a short path directly between X and B, and a longer path from X to B via A. It's essential that when
...
[archaeology of glitches](/post/2025-07-01-archaeology-of-glitchyes) posting.
...
the value of a node unchanged, that node's "update" should not be used to schedule further updates of nodes which might depende on it.
...
The diagram below shows the canonical situation, taken from Mokhov, Mitchell and Peyton Jones' 2018
...
canonical reactive system, Excel) are much the same. A user has changed a source file, `main.c`, in a way which
...
This might seem to be a quite basic variety of competence but in fact several reactive systems, such as knockout
